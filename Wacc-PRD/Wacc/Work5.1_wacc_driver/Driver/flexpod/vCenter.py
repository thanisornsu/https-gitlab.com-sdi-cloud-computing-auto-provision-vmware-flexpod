...
def upgradeCpu(vm_id, cpu_spec):
    if cpu_spec == "1" or cpu_spec == 1:
        socket_number = 1
    elif int(cpu_spec) % 2 == 0:
        socket_number = int(cpu_spec)/2
    else:
        socket_number = cpu_spec
    token_number = returnToken()
    url = "https://10.10.21.150/rest/vcenter/vm/{}/hardware/cpu".format(vm_id)

    payload = "{\n  \"spec\": {\n    \"count\": "+str(cpu_spec)+",\n    \"hot_remove_enabled\": false,\n    \"cores_per_socket\": "+str(socket_number)+",\n    \"hot_add_enabled\": true\n  }\n}"
    headers = {
...
def upgradeMemory(vm_id, mem_spec):
    token_number = returnToken()
    url = "https://10.10.21.150/rest/vcenter/vm/{}/hardware/memory".format(vm_id)
    mem_cal = int(mem_spec) * 1024
    payload = "{\n  \"spec\": {\n    \"size_MiB\": "+str(mem_cal)+"\n  }\n}"
    headers = {
        'Content-Type': "application/json",
        'vmware-api-session-id': "{}".format(token_number)
    }
