...
def get_org(cluster,vcloud_token):
    ...
    payload = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n<vmext:VMWProviderVdcParams\r\n " \
              ...
              "id=\"urn:vcloud:providervdcstorageprofile:c68c6439-c2ac-4f0e-9331-0a586c970734\" name=\"BTT_UCSD_POC_PUB_02\" type=\"application/vnd.vmware.admin.pvdcStorageProfile+xml\"/>\r\n   " \
...
def getVdcId(cluster,vcloud_token,org_id):
    url = "https://%s/api/org/%s" % (cluster['vcloud_ip'], org_id)
    headers = {
        'x-vcloud-authorization': vcloud_token,
        'Accept': "application/*+xml;version=31.0",
    ...
    if vdc_objs["Org"]["Link"] == {}:
        print "Never have vdc list"
        return "Don't have VDC name", "Don't have VDC Link"
    elif vdc_objs["Org"]["Link"] != {}:
        for i in range(len(vdc_objs["Org"]["Link"])):
            if vdc_objs["Org"]["Link"][i]["@type"] == "application/vnd.vmware.vcloud.vdc+xml":
                # vdc_list.append(vdc_objs["Org"]["Link"][i]["@name"])
                vdc_name = vdc_objs["Org"]["Link"][i]["@name"]
                vdc_link = vdc_objs["Org"]["Link"][i]["@href"]
        return vdc_name, vdc_link


def getVappId(vcloud_token, vdc_id):
    ...
    vapp_name = y["Vdc"]["ResourceEntities"]["ResourceEntity"]["@name"]
    vapp_link = y["Vdc"]["ResourceEntities"]["ResourceEntity"]["@href"]
    # vapp_id = y["Vdc"]["ResourceEntities"]["ResourceEntity"]["@href"].split("/")
    # print vapp_id[5]
    return vapp_name, vapp_link
...
def create_vdc_on_vcloud(cluster,vcloud_token,org_id,customer_name,zone, cpu, mem, disk):
    ...
    # if vdc_name not in vdc_list:
        cpu_math = int(math.ceil(int(cpu) * 1.2 * 2.6 * 1024))
        mem_math = int(math.ceil(int(mem) * 1.2 * 1024))
        cpu_math = str(cpu_math)
        mem_math = str(mem_math)
        # storage = int(disk + math.ceil(mem))
        ...
        payload = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" \
                ...
                "<ComputeCapacity>" \
                    "<Cpu>" \
                        "<Units>MHz</Units>" \
                        "<Allocated>"+cpu_math+"</Allocated>" \
                        "<Limit>"+cpu_math+"</Limit>" \
                    "</Cpu>" \
                    "<Memory>" \
                        "<Units>MB</Units>" \
                        "<Allocated>"+mem_math+"</Allocated>" \
                        "<Limit>"+mem_math+"</Limit>" \
                    "</Memory>" \
                "</ComputeCapacity>" \
                "<NicQuota>0</NicQuota>" \
...
def queryOrg():
    ...
    vcloud_token = create_sessions(cluster["vcloud_ip"], cluster["vcloud_username"], cluster["vcloud_password"])
    return_data, condition = get_org(cluster, vcloud_token)
    for sub_data in return_data:
        org_name = sub_data["org_name"]
        org_url = sub_data["org_url"]
        org_id = org_url.split("/")[5]
        return_data = {
            "org_name": org_name,
            "org_url": org_url,
            "org_id": org_id,
        }
        list_data.append(return_data)
    return list_data, condition


def queryVdc(organize_name):
    list_data = []
    cluster = getVcenterVcloudDetail("test")
    vcloud_token = create_sessions(cluster["vcloud_ip"], cluster["vcloud_username"], cluster["vcloud_password"])
    return_data, condition = get_org(cluster, vcloud_token)
    for sub_data in return_data:
        ...
        org_url = sub_data["org_url"]
        org_id = org_url.split("/")[5]
        if org_name != organize_name:
            pass
        elif org_name == organize_name:
            vdc_name, vdc_link = getVdcId(cluster, vcloud_token, org_id)
            vdc_id = vdc_link.split("/")[5]
            result_data = {
                "org_name": org_name,
                "org_url": org_url,
                "org_id": org_id,
        ...
        # else:
        #     return list_data.append("Doesn't have this organize name")
    return list_data


def queryVapp(organize_name, vdc_name_send):
    list_data = []
    cluster = getVcenterVcloudDetail("test")
    vcloud_token = create_sessions(cluster["vcloud_ip"], cluster["vcloud_username"], cluster["vcloud_password"])
    return_data, condition = get_org(cluster, vcloud_token)
    for sub_data in return_data:
        ...
        if org_name != organize_name:
            ...
            vdc_name, vdc_link = getVdcId(cluster, vcloud_token, org_id)
            vdc_id = vdc_link.split("/")[5]
            if vdc_name != vdc_name_send:
                pass
            elif vdc_name == vdc_name_send:
                vapp_name, vapp_url = getVappId(vcloud_token, vdc_id)
                vapp_id = vapp_url.split("/")[5]
                split_vapp_id = vapp_id.split("-")[1:]
                separator = "-"
                real_vapp_id = separator.join(split_vapp_id)
                result_data = {
                    "org_name": org_name,
                    "org_url": org_url,
                    "org_id": org_id,
                    ...
                    "vapp_name": vapp_name,
                    "vapp_url": vapp_url,
                    "vapp_id": real_vapp_id
                }
                list_data.append(result_data)
        #     else:
        #         return list_data.append("Doesn't have this vdc name")
        # else:
        #     return list_data.append("Doesn't have this organize name")
    return list_data
